...
def build_vocabulary(img_paths, vocab_size=400):
    ...
    vocab = kmeans(feature_list, num_centers=vocab_size)
...
def get_bags_of_sifts(img_paths, vocab):
    ...
    for img in img_paths:
        ...
        img = np.array(img)
        _, desc = dsift(img, step=[5, 5], fast = True)
        dist = cdist(desc, vocab)
        argmin = np.argmin(dist, axis=1)
        hist, _ = np.histogram(argmin, bins=np.arange(vocab.shape[0]+1), density=True)
...
def nearest_neighbor_classify(train_img_feats, train_labels, test_img_feats):
    ...
    ###########################################################################
    test_predicts = []
    for img in train_img_feats:
        img = img.reshape(1, -1)
        distance = cdist(img, train_img_feats, metric='minkowski', p=2)
        argmin = np.argsort(distance, axis=1)
        k = 5
        k_nearest = argmin[:, :k]
        k_labels = []
        for i in range(k_nearest.shape[0]):
            for j in range(k_nearest.shape[1]):
                k_label = train_labels[k_nearest[i][j].astype(int)]
                k_label = np.vectorize(CAT2ID.get)(k_label)
                k_labels.append(k_label)
        k_labels = np.array(k_labels)
        result = np.argmax(np.bincount(k_labels))    
        test_predicts.append(CAT[result])
